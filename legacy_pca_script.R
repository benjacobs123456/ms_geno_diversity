...
PC5 = rep(means$PC5,nrow(all_dat)),
PC6 = rep(means$PC6,nrow(all_dat)),
PC7 = rep(means$PC7,nrow(all_dat)),
PC8 = rep(means$PC8,nrow(all_dat)),
PC9 = rep(means$PC9,nrow(all_dat)),
PC10 = rep(means$PC10,nrow(all_dat))
))
...
png(paste0("./outputs/ukb_adams_pca_outliers_",ancestry,".png"),res=900,units="in",width=8,height=3)
...
mutate_at(.vars = vars(contains("PC")),z_score) %>%
left_join(all_cov %>% mutate(IID = as.character(IID)) %>% dplyr::select(-`#FID`),by="IID")
