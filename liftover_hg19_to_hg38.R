...
gwas_hg19_bed = gwas_hg19 %>%
  mutate(chr = paste0("chr",CHROM),bp1=GENPOS-1,bp2=GENPOS,snp = ID) %>%
...
dplyr::select(X4,X3) %>%
dplyr::rename("ID" = X4,"GENPOS_hg38" = X3)

# merge 
gwas_hg19_hg38 = gwas_hg19 %>%
  mutate("GENPOS_hg19" = GENPOS) %>%
  filter(ID %in% gwas_hg38$ID) %>%
  left_join(gwas_hg38,by="ID") %>%
  dplyr::select(-GENPOS) %>%
  dplyr::rename("GENPOS" = GENPOS_hg38) %>%
  mutate(ID = paste0(CHROM,":",GENPOS,":",ALLELE0,":",ALLELE1))
