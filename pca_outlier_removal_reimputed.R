...
# plot all PCs 
png(paste0("./outputs/reimputed_",out_ancestry,"_pc_pairs_plot.png"),res=900,units="in",width=12,height=12)
GGally::ggpairs(
    all_cov,
...
png(paste0("./outputs/reimputed_",out_ancestry,"_pc_eigenvals.png"),res=900,units="in",width=8,height=8)
...
# save plot 
png(paste0("./outputs/reimputed_",out_ancestry,"_pc_plot.png"),res=900,units="in",width=10,height=4)
print(cowplot::plot_grid(p,p2,p3,p4,p5,ncol=5,align="h"))
dev.off()

# repeat with UKB ethnicity 
ethnicity_dat = pre_filtering_dat %>% 
    inner_join(ukb_pheno_ethnicity %>% dplyr::rename("IID" = EID),
    by="IID")
abundant_groups = ethnicity_dat %>% dplyr::count(ethnic_background_f21000_0_0) %>% 
filter(!is.na(ethnic_background_f21000_0_0)) %>% 
slice_max(n,n=10)
ethnicity_dat = ethnicity_dat %>% 
    filter(ethnic_background_f21000_0_0 %in% abundant_groups$ethnic_background_f21000_0_0)
p1 = ggplot(ethnicity_dat,aes(PC1,PC2,col=ethnic_background_f21000_0_0))+
    geom_point(data = ethnicity_dat %>% filter(outlier == "outlier"),alpha=0.05)+
    geom_point(data = ethnicity_dat %>% filter(outlier != "outlier"),alpha=1)+
    theme_bw()+
    scale_color_brewer(palette="Paired")+
    labs(color="Ethnicity")
p2 = ggplot(ethnicity_dat,aes(PC3,PC4,col=ethnic_background_f21000_0_0))+
    geom_point(data = ethnicity_dat %>% filter(outlier == "outlier"),alpha=0.05)+
    geom_point(data = ethnicity_dat %>% filter(outlier != "outlier"),alpha=1)+
    theme_bw()+
    scale_color_brewer(palette="Paired")+
...
png(paste0("./outputs/reimputed_",out_ancestry,"_pc_ukb_ethnicity_plots.png"),res=900,units="in",width=8,height=4)
print(cowplot::plot_grid(p1,p2,ncol=2,align="h"))
dev.off()

# age & sex distros 
p1 = ggplot(all_cov %>% mutate(sex = ifelse(sex==2,"F","M")) %>% filter(!is.na(sex)),
    aes(cohort,fill=sex))+
    geom_bar(position="fill",color="black")+
    theme_bw()+
    scale_fill_brewer(palette="Set1")

p2 = ggplot(all_cov %>% filter(!is.na(age)),
    aes(age,fill=cohort))+
    geom_density(alpha=0.7,color="black")+
    theme_bw()
png(paste0("./outputs/reimputed_",out_ancestry,"_agesex_plots.png"),res=900,units="in",width=8,height=4)
print(cowplot::plot_grid(p1,p2,ncol=2,align="h"))
dev.off()
...
write_tsv(covars_all,paste0("./pheno/reimputed_",out_ancestry,"_covars_with_pcs.tsv"))
...
write_tsv(pheno_all,paste0("./pheno/reimputed_",out_ancestry,"_pheno.tsv"))
