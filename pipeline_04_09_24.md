...
# find just MS cases 
awk '{if($3=="MS") print $1,$1}' /data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/outputs/merged_ukb_adams_pheno_cov.tsv > /data/scratch/hmy117/ms_cases_only_for_case_case_gwas 

# restrict EUR samples to just these cases 
for i in {22..1};
    do 
        ~/plink --bfile /data/scratch/hmy117/filtered_genotypes_for_susceptibility_gwas_eur_hg19_chr$i \
        --chr $i \
        --keep /data/scratch/hmy117/ms_cases_only_for_case_case_gwas \
        --out /data/scratch/hmy117/filtered_genotypes_for_susceptibility_gwas_eur_just_ms_hg19_chr$i \
        --make-bed 
    done 

# set case-control status based on cohort 
awk '{if($2=="ADAMS") print $1,$1}' /data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/outputs/merged_ukb_adams_pheno_cov.tsv > /data/scratch/hmy117/ms_cases_adams_only_for_case_case_gwas 

# set phenotype and run fisher's exact test of case vs case 
# restrict EUR samples to just these cases 
for i in {22..1};
    do 
        ~/plink --bfile /data/scratch/hmy117/filtered_genotypes_for_susceptibility_gwas_eur_just_ms_hg19_chr$i \
        --chr $i \
        --make-pheno /data/scratch/hmy117/ms_cases_adams_only_for_case_case_gwas \* \
        --allow-no-sex \
        --out /data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/outputs/fisher_test_case_case_eur_gwas_chr$i \
        --assoc \
        --make-bed 
    done 
````
...
res = list()
for(i in c(1:22)){
    res[[i]] = read_table(paste0("/data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/outputs/fisher_test_case_case_eur_gwas_chr",i,".assoc"))
}
...
# save 
write_tsv(res %>% dplyr::select(SNP),"/data/scratch/hmy117/ms_cases_ukb_vs_adams_snps_to_exclude")

````


#### GWAS prep - running 05-09 for EUR
````unix 
...
# PLINK GWAS 
qsub "/data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/scripts/plink_gwas.sh" # running
...
#### GWAS within ancestry - using whole ancestral cluster 
````unix
...
#### HLA-TAPAS
````unix
qsub /data/home/hmy117/ADAMS/genotypes/QMUL_Aug_23/scripts/hla_imputation.sh # running 04-09
````
...
all_res = purrr::map(ancestries,function(x){
	gwas_res = read_plink_gwas(
		paste0("./outputs/plink_gwas_",x,".MS_status.glm.logistic.hybrid")) %>%
	mutate(anc = x) %>%
...
all_res = purrr::map(ancestries,function(x){
	gwas_res = read_regenie_gwas(
		paste0("./outputs/case_control_gwas_ALL_ANCESTRY_",x,"_MS_status.regenie")) %>%
	mutate(anc = x)
	gwas_res
})
